"""
file: power_supply/www.py
description: Power Supply module designed to control connected power supplies that communicate using the VISA protocol
author: Larry
"""
...
class ModuleWebGui:
	...
	def __init__(self, logger: logging.Logger, protocol: Protocol, module_folder_name: str):
		self.logger = logger
	...
	def _template_context(self, modules: list, embedded_mode: bool, current_path: str) -> dict:
		...
		return {
			"module_name": self.module_folder_name,
			"modules": modules,
	...
	def _create_flask_app(self) -> Flask:
		...
		def module_action():
			...
			try:
				self.protocol.send_action(self.module_folder_name, action, payload=arguments)
				return jsonify({"ok": True, "action": action})
			except Exception as exc:
				self.logger.exception("Failed action %s", action)
				return jsonify({"ok": False, "error": str(exc)}), 500

		@flask_app.post("/api/request")
		def module_request():
			body = request.get_json(silent=True) or {}
			action = body.get("action")
			arguments = body.get("arguments", {})
			...
			try:
				response = self.protocol.send_request(self.module_folder_name, action, payload=arguments, timeout=timeout_s)
				return jsonify({"ok": True, "response": response})
			except TimeoutError:
				return jsonify({"ok": False, "error": "Request timed out"}), 504
			except Exception as exc:
				self.logger.exception("Failed request %s", action)
				return jsonify({"ok": False, "error": str(exc)}), 500
